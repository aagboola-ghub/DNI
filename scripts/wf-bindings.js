...
function updateNumber(config, state){
  if (!window.__siteActivated){
    try { renderTrackingDetails(config, state, null); } catch(e){} 
    return;
  }
  let number = '';
  try { number = resolveNumber((config||{}).dniRules||[], state||{}); } catch(e){}
  try { renderTrackingDetails(config, state, number); } catch(e){}
  try { if (number && typeof applyToDom === 'function') applyToDom(number); } catch(e){}
}
...
on('call:end', ()=>{
  ...
  const medium = (state.campaignType === 'paid'
      ? (state.source==='facebook' ? 'paid_social' : 'cpc')
      : (state.source==='facebook' ? 'social' : 'organic'));
...
function renderTrackingDetails(config, state, number){
  // HARD SERP LOCK: nothing renders until a real click activates the site
  if (!window.__siteActivated){
  ...
  const mapSource = { google:'google', facebook:'facebook', microsoft:'bing' };
  const utmSource = mapSource[state.source] || state.source || 'direct';
  const utmMedium = state.campaignType === 'paid'
      ? (state.source === 'facebook' ? 'paid_social' : 'cpc')
      : (state.source === 'facebook' ? 'social' : 'organic');
  ...
  text('campaign-type', state.campaignType ? (state.campaignType==='paid'?'Paid Search':'Organic Search') : '—');
  text('tracking-mode', state.trackingMode ? (state.trackingMode==='channel'?'Channel Tracking':'Session Tracking') : '—');
  const tn = number || '—';
  const tEl = document.getElementById('tracking-number'); if (tEl) tEl.textContent = tn;
  const link = document.getElementById('tracking-number-link');
  if (link && number) link.setAttribute('href', 'tel:' + number.replace(/\D/g,''));
}
function updateNumber(config, state){
  if (!window.__siteActivated){ try { renderTrackingDetails(config, state, null); } catch(e){} return; }
  let number;
  try { number = resolveNumber((config||{}).dniRules||[], state||{}); } catch(e){ number = ''; }
  try { renderTrackingDetails(config, state, number); } catch(e){}
  try { if (number && typeof applyToDom === 'function') applyToDom(number); } catch(e){}
}
function setCallStatus(text){
