...
function updateNumber(config, state){
  ...
  applyToDom(number);
  renderTrackingDetails(config, state, number);
...
function renderTrackingDetails(config, state, number){
  ...
  if (typeof __journeyStage !== 'undefined' && __journeyStage === 'serp'){
  ...
  const sourceLabels = { google:'google', facebook:'facebook', microsoft:'microsoft' };
  const utmSource = sourceLabels[state.source] || state.source || 'direct';
  const utmMedium = state.campaignType === 'paid' ? 'cpc' : 'organic';
  ...
  const campaignTypeLabel = state.campaignType === 'paid' ? 'Paid Search' : 'Organic Search';
  const trackingTypeLabel = state.trackingMode === 'channel' ? 'Channel Tracking' : 'Session Tracking';
  const clickId = state.source === 'google' ? 'gclid=demo'
               : state.source === 'facebook' ? 'fbclid=demo'
               : state.source === 'microsoft' ? 'msclkid=demo' : '—';
...
(function(){
  ...
  document.addEventListener('click', (e)=>{
    ...
    if (txt.includes('wells fargo') || txt.includes('business checking')) return markSite();
  }, true);
  document.addEventListener('click', (e)=>{
    const b = e.target.closest('[data-nav-back], .back-to-results, #back-to-results');
    if (b){ setJourneyStage('serp'); }
  }, true);
})();
/*SERP->SITE DETECTOR*/
...
(function(){
  if (typeof on === 'function'){
    on('call:end', ()=>{
      try {
        const cfg = window.__cfg || {}; const state = window.__state || {};
        const sourceMap = { google:'google', facebook:'facebook', microsoft:'microsoft' };
        const medium = (state.campaignType === 'paid' ? 'cpc' : 'organic') || 'cpc';
        const attr = (cfg.attribution || {});
        const trackingText = (document.getElementById('tracking-number')||{}).textContent || '';
        const payload = {
