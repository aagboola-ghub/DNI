...
(function(){
  ...
  // Only count clicks on explicit SERP result links/buttons
  ...
  document.addEventListener('click', (e)=>{
    const a = e.target.closest(resultSelectors);
    if (a) return markSite();
  }, true);

  // Going back to results forces SERP stage
  document.addEventListener('click', (e)=>{
    const b = e.target.closest('[data-nav-back], .back-to-results, #back-to-results');
    if (b){
      setJourneyStage('serp');
      try { renderTrackingDetails(window.__cfg||{}, window.__state||{}, null); } catch(e){}
    }
  }, true);
})();
/*END SERP->SITE DETECTOR*/



/*SERP-ONLY DETECTOR: keep stage SERP when SERP markers are present; never promote to SITE*/
(function(){
  function isSerp(){
    try{
      if (document.querySelector('.serp, .serp-view, [data-page="serp"], .search-results')) return true;
      const bar = document.querySelector('.address-bar, .fake-url, .serp-url');
      if (bar && /www\.google\.com\/search\?/i.test(bar.textContent||'')) return true;
      // look for obvious serp-only chrome
      if (document.querySelector('form[role="search"], input[placeholder*="Search"]') && document.querySelector('.serp')) return true;
    }catch(e){}
    return false;
  }
  ...
  const mo = new MutationObserver(()=>{
    if (isSerp()){
      if (typeof setJourneyStage === 'function') {
        setJourneyStage('serp');
        try { renderTrackingDetails(window.__cfg||{}, window.__state||{}, null); } catch(e){}
      }
    }
