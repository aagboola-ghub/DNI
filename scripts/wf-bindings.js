...
function updateNumber(config, state){
  ...
  if (__journeyStage === 'site') { applyToDom(number); }
...
// Observe right-pane route changes (any major content swap sets SITE)
(function(){
  const root = document.querySelector('#app') || document.body;
  if (!root) return;
  const mo = new MutationObserver(()=>{
    const text = (document.querySelector('#right-pane,.site,.wf-site')||document.body).textContent || '';
    // Heuristic: if right pane contains obvious site-only terms, assume SITE
    if (/account|checking|open an account|small business|apply now/i.test(text)) {
      setJourneyStage('site');
    }
  });
...
function renderTrackingDetails(config, state, number){
  ...
  if (__journeyStage === 'serp'){
    ['traffic-source','utm-source','utm-medium','utm-campaign','utm-term','click-id','landing-page','original-number','tracking-number'].forEach(id=>{ const el=document.getElementById(id); if(el) el.textContent='—'; });
    const link = document.getElementById('tracking-number-link'); if (link) link.removeAttribute('href');
    text('campaign-type', state.campaignType === 'paid' ? 'Paid Search' : 'Organic Search');
    text('tracking-mode', state.trackingMode === 'channel' ? 'Channel Tracking' : 'Session Tracking');
    return;
  }
...
function renderCallAnalyticsCard(data){
  const host = document.getElementById('call-analytics-body');
  ...
  const items = [
    ...
    ["Caller's phone number", data.caller_number]
  ];
  host.innerHTML = items.map(([k,v])=>`<div class="d-flex justify-content-between border-bottom py-1"><div class="me-3"><strong>${k}</strong></div><div class="text-break">${v||''}</div></div>`).join('');
  card.style.display = '';
}
(function(){
  if (typeof on === 'function'){
    on('call:end', ()=>{
      try {
        const cfg = window.__cfg || {}; const state = window.__state || {};
        const sourceMap = { google:'google', facebook:'facebook', microsoft:'bing' };
        const medium = (state.campaignType === 'paid' ? (state.source==='facebook' ? 'paid_social' : 'cpc') : (state.source==='facebook' ? 'social' : 'organic'));
        const attr = (cfg.attribution || {});
        const trackingText = (document.getElementById('tracking-number')||{}).textContent || '';
        const payload = {
          attribution_details: {
        ...
        renderCallAnalyticsCard(payload);
      } catch(e){}
    });
  }
})();
